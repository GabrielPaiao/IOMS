...
export default function CriticalityBadge({ criticality, className = '' }: CriticalityBadgeProps) {
  const styles = {
  ...
  return (
    <span className={`inline-block text-xs px-2 py-0.5 rounded-full border ${styles[criticality]} ${className}`}>
      {criticality}
    </span>
  );
}